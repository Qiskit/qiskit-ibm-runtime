...
these items is a dictionary from strings to a ``np.ndarray`` with boolean ``dtype``. Let us take a
look at the three items in ``result`` to understand the meaning of their key-value pairs.
...
classical registers, stored in a ``np.ndarray`` of shape ``(1024, 3)``. The first axis
