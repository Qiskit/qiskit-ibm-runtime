...
class TestIntegrationIBMSampler(IBMIntegrationTestCase):
    ...
    def test_qasm2(self, service):
        ...
        with Session(service, self.backend) as session:
            sampler = Sampler(session=session)
            job = sampler.run(circuits=qasm2_program)
            result = job.result()
            self.assertEqual(len(result.quasi_dists), 1)
            self.assertEqual(len(result.metadata), 1)
            self.assertAlmostEqual(result.quasi_dists[0][0], 0.5, delta=0.1)
            self.assertAlmostEqual(result.quasi_dists[0][5], 0.5, delta=0.1)
    ...
    def test_qasm3_with_params(self, service):
        ...
        with Session(service, self.backend) as session:
            sampler = Sampler(session=session)
            job = sampler.run(circuits=qasm3_program, parameter_values=[[1.5, 1]])
            result = job.result()
            self.assertEqual(len(result.quasi_dists), 1)
            self.assertEqual(len(result.metadata), 1)
            self.assertAlmostEqual(result.quasi_dists[0][0], 0.25, delta=0.1)
            self.assertAlmostEqual(result.quasi_dists[0][1], 0.25, delta=0.1)
            self.assertAlmostEqual(result.quasi_dists[0][6], 0.25, delta=0.1)
    ...
    def test_qasm2_qc_combined(self, service):
        ...
        with Session(service, self.backend) as session:
            sampler = Sampler(session=session)
            job = sampler.run(circuits=[qasm2_program, circuit])
            result = job.result()
            self.assertEqual(len(result.quasi_dists), 2)
            self.assertEqual(len(result.metadata), 2)
            self.assertAlmostEqual(result.quasi_dists[0][0], 0.5, delta=0.1)
