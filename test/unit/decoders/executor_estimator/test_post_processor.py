...
@ddt
class TestCreatePubResultPea(unittest.TestCase):
    """Tests for the ``create_pub_result_pea`` function."""
    ...
    def test_no_meas_creg_pea(self):
        """Test that item result without ``'_meas'`` key raises for create_pub_result_pea."""
        data = np.zeros((2, 1, 1, 10, 2), dtype=bool)
        item_result = QuantumProgramItemResult({"meas": data})
        with self.assertRaisesRegex(ValueError, "Dedicated creg ``'_meas'``"):
            create_pub_result_pea(
    ...
    def test_ndim_raises_pea(self):
        """Test that item result with invalid ndim (not 5) raises for create_pub_result_pea."""
        # 4D data (as used in PEC) instead of 5D should raise
        data = np.zeros((1, 1, 10, 2), dtype=bool)
        item_result = QuantumProgramItemResult({"_meas": data})
        with self.assertRaisesRegex(ValueError, "has ``4`` axes, expected ``5``"):
            create_pub_result_pea(
                item_result=item_result,
    ...
    def test_evs_noiseless_pea(self):
        """Test that noiseless data (all zeros) extrapolated to zero noise gives 1.0.

        Uses ZZ observable with all-zero measurements at two noise factors.
        A linear fit of [1.0, 1.0] extrapolated to 0 should remain 1.0.
        The result evs array leading axis equals len(extrapolated_noise_factors).
        """
        # Shape: (num_noise_scales=2, num_randomizations=1, num_configs=1, shots=10, num_qubits=2)
        data = np.zeros((2, 1, 1, 10, 2), dtype=bool)
        item_result = QuantumProgramItemResult({"_meas": data})
        ...
        # leading axis is len(extrapolated_noise_factors) = 1
        self.assertEqual(evs.shape[0], len(extrapolated_noise_factors))
        self.assertAlmostEqual(float(evs[0]), 1.0, places=5)

    def test_evs_noisy_linear_extrapolation_pea(self):
        """Test that linear extrapolation recovers the zero-noise value from noisy data.

        ZZ observable. Measurements at noise_factor=1 give ev=1.0 (all 00),
        at noise_factor=2 give ev=0.0 (half flipped). Linear fit: ev = 2 - x,
        so at x=0 the extrapolated value is 2.0.
        """
        # At noise factor 1: all 00 -> ev = +1
        data_nf1 = np.zeros((1, 1, 10, 2), dtype=bool)
        # At noise factor 2: ev = 0
        # For ZZ: 00->+1, 01->-1; flip qubit1 on 5 shots out of 10 -> ev=0
        data_nf2 = np.zeros((1, 1, 10, 2), dtype=bool)
        data_nf2[0, 0, 5:10, 1] = True  # 5 shots with bit1 flipped -> 01 -> -1

        # Stack: shape (num_noise_scales=2, num_randomizations=1, num_configs=1, shots=10, qubits=2)
        data = np.stack([data_nf1, data_nf2], axis=0)
        item_result = QuantumProgramItemResult({"_meas": data})
        ...
        # linear fit through (1, 1.0) and (2, 0.0): slope=-1, intercept=2 → at x=0: ev=2.0
        self.assertAlmostEqual(float(evs[0]), 2.0, places=5)

    def test_evs_2d_obs_no_params_pea(self):
        """Test create_pub_result_pea with 2D observables and no params."""
        # Two configs: one for ZZ, one for XX (all 00 measurements at two noise factors)
        # Shape: (num_noise_scales=2, num_randomizations=1, num_configs=2, shots=10, num_qubits=2)
        data = np.zeros((2, 1, 2, 10, 2), dtype=bool)
        item_result = QuantumProgramItemResult({"_meas": data})
        ...
        # All-zero measurements give ev=+1 at both noise factors; linear fit stays 1.0 at x=0
        expected = np.ones(observables.shape)
        self.assertTrue(np.allclose(evs[0], expected), msg=f"Expected {expected}, got {evs[0]}")
    ...
    @unpack
    def test_evs_values_without_twirling_pea(self, obs_shape, param_shape, expected_evs_base):
        """Test the correctness of evs when twirling is OFF with PEA.

        Expects shapes that broadcast into ``(4,)``.
        """
        # 4 non-commuting observables -> always 4 configs
        obs_like = [{"000": 1 / 2, "111": 1 / 2}, {"+++": 1}, {"rrr": 1}, {"+r0": 1}]
        observables = ObservablesArray(obs_like).reshape(obs_shape)

        num_noise_scales = 2
        # Shape: (num_noise_scales, num_randomizations=1, num_configs=4, shots=10, num_qubits)
        data = np.zeros((num_noise_scales, 1, 4, 10, observables.num_qubits), dtype=bool)
        item_result = QuantumProgramItemResult({"_meas": data})
        ...
        # All-zero data at every noise factor → flat linear fit → evs[0] == expected_evs_base
        self.assertTrue(
            np.allclose(evs[0], expected_evs_base),
            msg=f"Expected {expected_evs_base}, got {evs[0]}",
        )
    ...
    @unpack
    def test_evs_values_with_twirling_pea(self, obs_shape, param_shape, expected_evs_base):
        """Test the correctness of evs when twirling is ON with PEA.

        When all measurement data equals the flips (XOR cancels to zero), the ideal
        expectation value is recovered. A linear fit of identical values extrapolated to 0
        ...
        data_shape = (num_noise_scales, 18, 4, 10, observables.num_qubits)
        flips = np.random.randint(0, 2, size=data_shape).astype(bool)
        # twirled_data == flips so XOR cancels => underlying measurements are all zero
        twirled_data = flips
        item_result = QuantumProgramItemResult(
            {"_meas": twirled_data, "measurement_flips._meas": flips}
        )
    ...
    def test_evs_shape_with_non_trivial_broadcasting_pea(self, obs_shape, param_shape):
        ...
        expected_broadcast_shape = np.broadcast_shapes(obs_shape, param_shape)
        # evs, stds and ensemble_standard_error all have a leading axis for num extrapolated factors
        expected_evs_shape = (len(extrapolated_noise_factors),) + expected_broadcast_shape
        self.assertTupleEqual(pub_result.data.evs.shape, expected_evs_shape)
        self.assertTupleEqual(pub_result.data.stds.shape, expected_evs_shape)
        self.assertTupleEqual(pub_result.data.ensemble_standard_error.shape, expected_evs_shape)

    def test_multiple_extrapolated_noise_factors_pea(self):
        """Test that multiple extrapolated noise factors produce the correct leading-axis size."""
        # Shape: (num_noise_scales=3, num_randomizations=1, num_configs=1, shots=10, num_qubits=2)
        data = np.zeros((3, 1, 1, 10, 2), dtype=bool)
        item_result = QuantumProgramItemResult({"_meas": data})
        ...
        # Leading dimension equals number of extrapolation points
        self.assertEqual(evs.shape[0], len(extrapolated_noise_factors))
        self.assertEqual(stds.shape[0], len(extrapolated_noise_factors))
        # All-zero measurements at every noise factor: flat linear fit stays 1.0 everywhere
        self.assertTrue(np.allclose(evs, 1.0), msg=f"Expected all 1.0, got {evs}")

    def test_stds_without_twirling_pea(self):
        """Test that stds and ensemble_standard_error are equal without twirling."""
        # Shape:
        # (num_noise_scales=2, num_randomizations=1, num_configs=1, shots=10, num_qubits=2)
        data = np.zeros((2, 1, 1, 10, 2), dtype=bool)
        # Mix a few flipped shots to get a non-trivial ev
        data[0, 0, 0, 8:10, 1] = True  # nf=1: ev = 0.6
        item_result = QuantumProgramItemResult({"_meas": data})
        ...
        # With num_randomizations=1, stds must equal ensemble_standard_error
        self.assertAlmostEqual(
