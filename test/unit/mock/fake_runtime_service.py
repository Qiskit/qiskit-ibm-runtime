...
class FakeRuntimeService(QiskitRuntimeService):
    ...
    def __init__(self, *args, num_hgps=2, runtime_client=None, backend_specs=None, **kwargs):
        ...
        if not isinstance(self._api_client, BaseFakeRuntimeClient):
            self._api_client = self._fake_runtime_client or BaseFakeRuntimeClient(backend_specs=self._backend_specs)
    ...
    def _set_api_client(self, hgps):
        ...
        if not self._fake_runtime_client:
            if not self._backend_specs:
                self._backend_specs = [ FakeApiBackendSpecs(
                    backend_name=self.DEFAULT_COMMON_BACKEND,
                    hgps=hgps
                )]
                for idx, hgp in enumerate(hgps):
                    self._backend_specs.append(
                        FakeApiBackendSpecs(backend_name=self.DEFAULT_UNIQUE_BACKEND_PREFIX+str(idx), hgps=[hgp])
                    )
            self._fake_runtime_client = BaseFakeRuntimeClient(backend_specs=self._backend_specs, channel="ibm_quantum")
