...
def passthrough_data_to_2_0(passthrough_data: DataTree) -> DataTreeModel:
    """Convert passthrough data to schema model."""
    if isinstance(passthrough_data, dict):
        return {k: passthrough_data_to_2_0(v) for k, v in passthrough_data.items()}
    if isinstance(passthrough_data, (list, tuple)):
        return [passthrough_data_to_2_0(v) for v in passthrough_data]
    if isinstance(passthrough_data, np.ndarray):
        return CompressedTensorModel.from_numpy(passthrough_data)
    return passthrough_data


def passthrough_data_from_2_0(passthrough_data: DataTree) -> DataTreeModel:
    """Convert passthrough data to schema model."""
    if isinstance(passthrough_data, CompressedTensorModel):
        return passthrough_data.to_numpy()
    if isinstance(passthrough_data, dict):
        return {k: passthrough_data_from_2_0(v) for k, v in passthrough_data.items()}
    if isinstance(passthrough_data, list):
        return [passthrough_data_from_2_0(el) for el in passthrough_data]
    return passthrough_data
...
def quantum_program_from_2_0(model: ParamsModel) -> tuple[QuantumProgram, ExecutorOptions]:
    ...
    quantum_program = QuantumProgram(
        ...
        passthrough_data=passthrough_data_from_2_0(program_model.passthrough_data),
    )
