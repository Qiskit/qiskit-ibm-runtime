...
`IBM Quantum platform migration <https://docs.quantum.ibm.com/migration-guides/classic-iqp-to-cloud-iqp>`__:
    ...
    - When initializing the service, there is a new channel type, ``ibm_quantum_platform``, which will replace ``ibm_cloud``. 
    ...
    There are new parameters ``region``, and ``plans_preference``, that can be used to prioritize certain instances. Note that 
